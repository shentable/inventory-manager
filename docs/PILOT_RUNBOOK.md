...
店长/管理员在首页“消耗与补货”查看库存预测；店员无入口和接口权限。
开始使用时，保持库存单位一致，并及时登记到货、报损；盘点尽量在闭店后完成并及时确认。
两个有效双人确认结果之间至少间隔一天，才产生一个可用周期。无历史时看板显示“待核查”，不编造用量。
“更新测算”可调整自然日统计范围、到货等待天数和到货后覆盖天数；这些参数不修改库存品资料。
每天优先处理“今日现场报缺”和“建议补货”，再检查“待核查”“临期核实”；点击品项的计算依据可追溯确认结果编号。
采购前先核对已下单未到货数量，按实际包装规格及供应批次效期决定采购量。看板预测不自动扣账或下单。
若确认提示盘点之后已有库存变动，请退回重盘或由原提交人现场重新核对后更新记录，再重新比对。

## 5. 恢复和升级

数量版本 `20260908_09` 支持所有单位的一位小数。报损、盘点、入库、采购和最低库存都可输入如 `0.3`、`2.5`，
整数显示为 `2`。报损库存小于 1 个单位时仍可登记；跨批次及连续小数扣减保持十分位精度。
升级会将数据库内部数量改为整数十分位，API 和页面中的实际数量保持不变。升级前备份；
新库出现小数数量后不能降级为整数库存版本，应通过升级前备份恢复，避免截断库存或历史流水。
